...
chsloka(){
 ...
 [ $new -eq 1 ] && ffmpeg -i chapter-name.wav -i sloka.wav -filter_complex '[0:0][1:0]concat=n=2:v=0:a=1[out]' -map '[out]'  current.wav
...
mp3wav chapter-name.mp3 chapter-name.wav
...
do
  ...
  mp3wav audio-pp.mp3 audio-pp.wav
  mp3wav audio-tt.mp3 audio-tt.wav
  mp3wav audio-ss.mp3 audio-ss.wav
  # mp3wav audio-chsl.mp3 audio-chsl.wav
  ...
  if [  $mp3cat -eq 0 ]; then
    ...
    if [ $new -eq 0 ]; then
      ...
      [ $param == "S-P-T-S" ] && ffmpeg -i ch-sloka.wav -i audio-ss.wav -i audio-pp.wav -i audio-tt.wav -i audio-ss.mp3 -filter_complex '[0:0][1:0][2:0][3:0][4:0]concat=n=5:v=0:a=1[out]' -map '[out]' current.wav
    ...
    MEENU=1 #JUST TO IGNORE THE ABOVE LINE
  fi
  ...
  # MEENU
  # ffmpeg -i ch-sloka.wav -i audio-ss.wav -i audio-pp.wav -i audio-tt.wav -i audio-ss.mp3 -filter_complex '[0:0][1:0][2:0][3:0][4:0]concat=n=5:v=0:a=1[out]' -map '[out]' current.wav
  rm -f current.wav
  ffmpeg -i audio-ss.wav -i audio-pp.wav -i audio-ss.mp3 -i audio-ss.mp3 -i audio-ss.mp3 -filter_complex '[0:0][1:0][2:0][3:0][4:0]concat=n=5:v=0:a=1[out]' -map '[out]' current.wav
...
mkdir -p /Volumes/SREE256GB/UNWANTED/GITA-MEENU/
cd /Volumes/SREE256GB/UNWANTED/GITA-MEENU/
